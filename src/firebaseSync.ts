      ...
      if (!docSnap.exists()) {
        ...
        if (providedToken) {
          ...
          const emergencyAttendee: Attendee = {
            ...
            email: "recovered@symposium.com",
            phone: "0000000000",
            college: "Engineering College",
            branch: "CSE",
            year: "Participant",
            registeredEventId: "symposium_event",
            eventId: "symposium_event",
            registeredEventTitle: "Symposium Event",
            regType: "individual",
            checked_in: true,
...
export async function hostCheckInParticipantTransaction(
  ...
  try {
    ...
    // Safely update event stats outside transaction to prevent transaction read/write ordering issues
    if (result.success) {
      ...
      if (docSnap.exists()) {
        ...
        if (participantEventId) {
          try {
            ...
            if (eventSnap.exists()) {
              await updateDoc(eventRef, { checkedInCount: increment(1) });
            } else {
            ...
            console.warn("Non-fatal: Failed to update event checkedInCount stats", eventErr);
          }
        }
      }
    }
