...
export default function ParticipantProfile({ 
  ...
  const handleRevoke = () => {
    if (confirm(`CRITICAL WARNING: Are you sure you want to permanently revoke registration for ${attendee.name} (${attendee.id})? This action cannot be undone.`)) {
      // Clean up offline queue so it doesn't get synced back
      const pendingOpsStr = localStorage.getItem('ai_symposium_offline_queue');
      if (pendingOpsStr) {
        try {
          const ops = JSON.parse(pendingOpsStr);
          const filteredOps = ops.filter((op: any) => op.id !== attendee.id);
      ...
      if (cachedStr) {
        try {
          ...
          const filteredCache = cached.filter((a: any) => a.id !== attendee.id);
