...
import { 
  ...
  Plus, Trash2, Send, Code
} from 'lucide-react';

const EventSkeletonCard = () => (
  <div className="animate-pulse p-5 rounded-2xl border border-outline-variant bg-surface-container-low space-y-4">
    <div className="flex justify-between items-start">
      <div className="w-10 h-10 bg-outline-variant/40 rounded-xl" />
      <div className="flex flex-col gap-1 items-end">
        <div className="w-16 h-3 bg-outline-variant/40 rounded-full" />
        <div className="w-12 h-3 bg-outline-variant/40 rounded-full mt-1" />
      </div>
    </div>
    <div className="h-5 bg-outline-variant/45 rounded-md w-2/3" />
    <div className="h-3 bg-outline-variant/45 rounded-md w-full" />
    <div className="h-3 bg-outline-variant/45 rounded-md w-5/6" />
    <div className="border-t border-outline-variant/20 pt-3 space-y-2">
      <div className="h-3 bg-outline-variant/40 rounded-md w-1/2" />
      <div className="h-3 bg-outline-variant/40 rounded-md w-1/3" />
    </div>
  </div>
);
...
export default function PublicRegistration({ 
  ...
  // Explicit Full-Day Events configuration
  const FULL_DAY_EVENT_IDS = new Set(['paper_presentation', 'project_expo', 'freefire', 'dsa_coding_challenge']);

  const isEventFullDay = (ev: SymposiumEvent) => {
    if (FULL_DAY_EVENT_IDS.has(ev.id)) return true;
    const s = (ev.session || '').toLowerCase();
    return s.includes('full-day') || s.includes('full day');
  };

  // Dynamic session classification helpers
  const isMorningSession = (ev: SymposiumEvent) => {
    if (isEventFullDay(ev)) return true;
    const s = (ev.session || '').toLowerCase();
    return s.includes('morning');
  };

  const isAfternoonSession = (ev: SymposiumEvent) => {
    if (isEventFullDay(ev)) return true;
    const s = (ev.session || '').toLowerCase();
    return s.includes('afternoon');
  };
  ...
  const getEventTime = (ev: SymposiumEvent) => {
    if (isEventFullDay(ev)) return '9:00 AM – 4:30 PM (Full Day)';
    const s = (ev.session || '').toLowerCase();
  ...
  const handleSelectEvent = (ev: SymposiumEvent, slot: 'morning' | 'afternoon') => {
    const isFullDay = isEventFullDay(ev);
  ...
  const isMorningDisabled = (ev: SymposiumEvent) => {
    ...
    if (afternoonEventId !== '') {
      const afternoonSelected = events.find(e => e.id === afternoonEventId);
      if (afternoonSelected && isEventFullDay(afternoonSelected)) return true;
    }
    return false;
  };
  ...
  const isAfternoonDisabled = (ev: SymposiumEvent) => {
    ...
    if (morningEventId !== '') {
      const morningSelected = events.find(e => e.id === morningEventId);
      if (morningSelected && isEventFullDay(morningSelected)) return true;
    }
    return false;
  };

  const maxAllowedMembers = selectedEvents.length > 0 
    ? Math.min(...selectedEvents.map(ev => ev.maximumTeamSize || 4)) 
    : 4;
  const minAllowedMembers = selectedEvents.length > 0 
    ? Math.max(...selectedEvents.map(ev => ev.minimumTeamSize || 1)) 
    : 1;

  const maxExtraMembers = Math.max(0, maxAllowedMembers - 1);
  const minExtraMembers = Math.max(0, minAllowedMembers - 1);
  ...
  // Pre-fill minimum required team member inputs when Team is selected or required
  React.useEffect(() => {
    if (regType === 'team') {
      const targetMin = Math.max(1, minExtraMembers);
      if (teamMembersInput.length < targetMin) {
        const needed = targetMin - teamMembersInput.length;
        const newInputs = Array.from({ length: needed }).map(() => ({ name: '', phone: '', email: '' }));
        setTeamMembersInput(prev => [...prev, ...newInputs]);
      } else if (teamMembersInput.length > maxExtraMembers) {
        setTeamMembersInput(prev => prev.slice(0, maxExtraMembers));
      }
    }
  }, [regType, minExtraMembers, maxExtraMembers]);
  ...
  const handleAddMember = () => {
    if (teamMembersInput.length >= maxExtraMembers) {
      alert(`Maximum of ${maxAllowedMembers} team members allowed (1 Team Leader + ${maxExtraMembers} Member${maxExtraMembers === 1 ? '' : 's'}).`);
      return;
    }
  ...
  const handleRemoveMember = (idx: number) => {
    if (teamMembersInput.length <= minExtraMembers) {
      alert(`At least ${minAllowedMembers} team members (1 Team Leader + ${minExtraMembers} Member${minExtraMembers === 1 ? '' : 's'}) are required for this event.`);
      return;
    }
    ...
    try {
      ...
      const templates = selectedEventsToCreate.map(ev => {
        return {
          ...
          checked_in: false,
          checked_in_at: null,
          lunch_status: 'NOT_CHECKED_IN',
          lunch_redeemed_at: null,
    ...
    <div className={`w-full min-h-screen bg-background text-on-background flex flex-col items-center ${isSpotRegistration ? 'p-0' : 'py-4 md:py-8'}`}>
      {!isSpotRegistration && (
        <header className="w-full max-w-2xl px-4 md:px-6 py-3 md:py-4 flex items-center justify-between mb-2 md:mb-4">
          ...
          <div className="flex items-center gap-3">
            {isOnline === false ? (
              <span className="px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-bold flex items-center gap-1 border border-amber-500/20">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
                Offline Mode
              </span>
            ) : (
              <span className="px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold flex items-center gap-1 border border-emerald-500/20">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-6 pt-4 border-t border-outline-variant/30">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <div className="px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 text-primary text-xs font-bold self-start sm:self-auto flex items-center gap-1.5 shadow-xs">
                    <span>Registration Fee: ₹100 / person per event</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {isLoadingEvents || morningEvents.length === 0 ? (
                      Array.from({ length: 4 }).map((_, i) => <EventSkeletonCard key={`morning-sk-${i}`} />)
                    ) : (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {isLoadingEvents || afternoonEvents.length === 0 ? (
                      Array.from({ length: 4 }).map((_, i) => <EventSkeletonCard key={`afternoon-sk-${i}`} />)
                    ) : (
                    ...
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-on-surface-variant font-semibold mt-0.5">
                          {minAllowedMembers === maxAllowedMembers 
                            ? `Strictly ${maxAllowedMembers} Team Members Required (1 Leader + ${maxExtraMembers} Member${maxExtraMembers === 1 ? '' : 's'})`
                            : `Team Size: ${minAllowedMembers}–${maxAllowedMembers} Members (1 Leader + ${minExtraMembers} to ${maxExtraMembers} Member${maxExtraMembers === 1 ? '' : 's'})`
                          }
                        </p>
                      </div>
                      <button
                        ...
                        disabled={teamMembersInput.length >= maxExtraMembers}
