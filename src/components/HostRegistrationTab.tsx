...
export default function HostRegistrationTab({ hostAssignedEventId, attendees }: HostRegistrationTabProps) {
  ...
  const handleScanTrigger = async (text: string) => {
    ...
    try {
      ...
      const numMembers = primaryMatch?.memberCount || (primaryMatch?.teamMembers?.length ? primaryMatch.teamMembers.length + 1 : 1);
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      let someSuccess = false;
      let lastFailMessage = 'Failed to update payment status.';
      for (const att of statusDetails.allAttendees) {
        const pid = att.participantId || att.id;
      ...
      if (someSuccess) {
        ...
        alert(lastFailMessage); // Fallback alert for payment issues
      }
    } catch (err: any) {
              ...
              <div className="w-full max-w-2xl bg-surface border border-outline-variant shadow-xl rounded-2xl overflow-hidden text-left relative z-10">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      {(statusDetails?.numMembers > 1 || statusDetails?.attendee?.teamMembers?.length > 0 || statusDetails?.attendee?.teamName) && (
                        <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-xl">
                          <p className="text-xs font-bold text-amber-700 uppercase tracking-wider mb-1 flex items-center gap-1">
                            <Users className="w-4 h-4" /> Team Details (Total: {statusDetails?.numMembers || (statusDetails?.attendee?.teamMembers?.length ? statusDetails.attendee.teamMembers.length + 1 : 1)} Members)
                          </p>
                          ...
                          <p className="text-sm font-bold text-amber-700/80 mt-1">
                            Team Leader: {statusDetails?.attendee?.name}
                          </p>
                          {statusDetails?.attendee?.teamMembers && statusDetails.attendee.teamMembers.length > 0 && (
                            <p className="text-sm font-bold text-amber-700/80 mt-0.5">
                      ...
                      <div className="bg-surface-container border border-outline-variant p-4 rounded-xl">
                        ...
                        {statusDetails?.attendee?.paymentStatus === 'Paid' ? (
                          <div className="bg-green-500/10 text-green-700 px-4 py-3 rounded-lg font-bold flex items-center justify-between gap-2">
                            ...
                            {statusDetails?.totalAmount !== undefined && (
                              <span className="text-sm font-black">
                                ₹{statusDetails.totalAmount} ({statusDetails.numMembers || 1} Member{(statusDetails.numMembers || 1) === 1 ? '' : 's'})
                              </span>
                            )}
                          </div>
                        ) : (
                          <div className="space-y-3">
                            <div className="bg-amber-500/10 text-amber-700 px-4 py-3 rounded-lg font-bold flex items-center justify-between gap-2">
                              ...
                              {statusDetails?.totalAmount !== undefined && (
                                <span className="text-sm font-black">
                                  ₹{statusDetails.totalAmount} Due ({statusDetails.numMembers || 1} Member{(statusDetails.numMembers || 1) === 1 ? '' : 's'})
                                </span>
                              )}
                            </div>
                            <button
                              ...
                              disabled={loading}
                              className="w-full bg-on-surface text-surface py-2 rounded-lg font-bold text-sm hover:bg-on-surface/90 disabled:opacity-50 cursor-pointer"
                            >
