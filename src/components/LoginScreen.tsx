...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isDefaultSuperadmin = (trimmedEmail === 'superadmin@gmail.com');
      const isOfficialHost = (trimmedEmail in MAP_EMAIL_TO_EVENT_ID);
      const isRegistrationTeam = (trimmedEmail === 'registration@aitheronml.in');
      ...
      if (isDefaultSuperadmin || isOfficialHost || isRegistrationTeam) {
        ...
        try {
          ...
          console.warn("Auto registration check", regErr);
          if (regErr.code === 'auth/email-already-in-use') {
            try {
              await signInWithEmailAndPassword(auth, trimmedEmail, trimmedPassword);
              authSuccess = true;
            } catch (signInErr) {
              console.warn("Sign-in failed for existing account with provided password", signInErr);
              // Do NOT grant authSuccess if the password was incorrect!
            }
          }
        }
      ...
      if (err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        msg = 'Incorrect password. Please check your credentials and try again.';
