    ...
    if (this.state.hasError) {
      return (
        <div style={{ display: 'flex', minHeight: '100vh', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0a0a0a', color: '#ff3e6c', fontFamily: 'monospace', padding: '20px', textAlign: 'center' }}>
          ...
          <p style={{ marginBottom: '20px', color: '#888' }}>{this.state.error?.message || 'An unexpected error occurred.'}</p>
          <button 
            onClick={() => {
...
function AppContent() {
      ...
      if (firebaseUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Always trust MAP_EMAIL_TO_EVENT_ID for known host emails.
            // Stale or missing Firestore records must not override the canonical mapping.
            const canonicalEventId = MAP_EMAIL_TO_EVENT_ID[email] || ((dbAssignedEventId && dbAssignedEventId !== 'none') ? dbAssignedEventId : assignedEventId);
            setSession({
              email: email,
              role: (data.role || role) as 'superadmin' | 'host' | 'registration',
              name: MAP_EMAIL_TO_NAME[email] || data.name || name,
              assignedEventId: canonicalEventId
            });
          } else {
    ...
    async function initRealtimeData() {
      ...
      try {
        ...
        unsubscribeEvents = subscribeWithRetry('events', (snapshot) => {
          ...
          // Always merge and enforce official INITIAL_EVENTS definitions (session, team sizes, title, hosts) over Firestore
          const mergedEvents = INITIAL_EVENTS.map(initEv => {
            const fbEv = fbEvents.find(e => e.id === initEv.id);
            if (fbEv) {
              return {
                ...fbEv,
                session: initEv.session,
                minimumTeamSize: initEv.minimumTeamSize,
                maximumTeamSize: initEv.maximumTeamSize,
                title: initEv.title,
                eventName: initEv.eventName,
                hostName: initEv.hostName,
                hostEmail: initEv.hostEmail
              };
            }
            return initEv;
          });
          ...
          localStorage.setItem('ai_symposium_events', JSON.stringify(mergedEvents));
          setIsLoading(false);
        }, (error) => {
  ...
  const handleLogin = (email: string, role: 'superadmin' | 'host' | 'registration', name: string, assignedEventId?: string) => {
    ...
    const resolvedEventId = MAP_EMAIL_TO_EVENT_ID[normalized] || assignedEventId || '';
  ...
  // Protective Route Checks with Error Cards to ELIMINATE Infinite Redirect Loops
    ...
    if (session.role !== 'superadmin') {
      return (
        <div className="min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-4 font-sans">
          <div className="max-w-md w-full bg-surface border border-outline-variant p-8 rounded-2xl text-center space-y-4 shadow-lg">
            <h2 className="text-xl font-bold text-red-500">Access Restricted</h2>
            <p className="text-sm text-on-surface-variant">Your current account ({session.email}) does not have Super Admin permissions.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-semibold cursor-pointer">Sign Out & Switch Account</button>
          </div>
        </div>
      );
    }
  ...
  const requireRegistration = (element: React.ReactNode) => {
    if (!session) return <Navigate to="/login" replace />;
    if (session.role !== 'registration' && session.role !== 'superadmin') {
      return (
        <div className="min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-4 font-sans">
          <div className="max-w-md w-full bg-surface border border-outline-variant p-8 rounded-2xl text-center space-y-4 shadow-lg">
            <h2 className="text-xl font-bold text-red-500">Access Restricted</h2>
            <p className="text-sm text-on-surface-variant">Your current account ({session.email}) does not have Registration Desk permissions.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-semibold cursor-pointer">Sign Out & Switch Account</button>
          </div>
        </div>
      );
  ...
  const requireHost = (eventId: string, element: React.ReactNode) => {
    if (!session) return <Navigate to="/login" replace />;
    const canonicalEventId = MAP_EMAIL_TO_EVENT_ID[session.email.toLowerCase()] || session.assignedEventId;
    if (session.role !== 'host' || (canonicalEventId && canonicalEventId !== eventId)) {
      return (
        <div className="min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-4 font-sans">
          <div className="max-w-md w-full bg-surface border border-outline-variant p-8 rounded-2xl text-center space-y-4 shadow-lg">
            <h2 className="text-xl font-bold text-red-500">Host Access Restricted</h2>
            <p className="text-sm text-on-surface-variant">Your account ({session.email}) is assigned to event <strong>{canonicalEventId || 'unknown'}</strong>, not this panel.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-semibold cursor-pointer">Sign Out & Switch Account</button>
          </div>
        </div>
      );
    ...
    <div id="symposium-app-root" className="min-h-screen bg-background text-on-background selection:bg-primary/20">
      <React.Suspense fallback={<LoadingFallback />}>
        <Routes>
          ...
          <Route path="/dsa" element={requireHost('dsa_coding_challenge', (
            <HostDashboard 
              user={session!}
              events={events}
              attendees={attendees}
          ...
          <Route path="/" element={
            session ? (
              ...
              <Navigate to={Object.keys(ROUTE_TO_EVENT_ID).find(k => ROUTE_TO_EVENT_ID[k] === (MAP_EMAIL_TO_EVENT_ID[session.email.toLowerCase()] || session.assignedEventId)) || "/dsa"} replace />
            ) : (
